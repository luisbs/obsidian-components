...
export class SettingsTabComponents {
  ...
  #toggleComponents(ids: string[], enabled: boolean): void {
    const components = this.#plugin.settings.components_config //
      .map((item) => (ids.includes(item.id) ? { ...item, enabled } : item))
  ...
  #displayComponentsHeader(parentEl: HTMLElement): void {
    ...
    el.addSearch((input) => {
      ...
      input.onChange((filter) => {
        ...
        for (const component of this.#plugin.settings.components_config) {
          const isIncluded = filter ? component.path.contains(filter) : true
  ...
  #componentForm(setting: Setting, c: ComponentConfig): void {
    ...
    el.addButton((button) => {
      ...
      button.onClick(() => {
        this.#changeComponent(c.id, 'names', formInput?.getValue() || '')
      })
    })
  }
}
